 ...
 */
import { describeTextQuote as abstractDescribeTextQuote } from '../../selector/text/describe-text-quote.js';
 ...
 * @public
 */
export async function describeTextQuote(range, scope, options = {}) {
    const scopeAsRange = toRange(scope ?? ownerDocument(range));
    const chunker = new TextNodeChunker(scopeAsRange);
    return await abstractDescribeTextQuote(chunker.rangeToChunkRange(range), () => new TextNodeChunker(scopeAsRange), options);
}
