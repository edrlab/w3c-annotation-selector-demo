...
import { nodeLength, nextNodeInShadowIncludingTreeOrder, isShadowIncludingDescendant, isShadowIncludingInclusiveAncestor, substringData, } from './whatwg-dom.js';
import { languageOf, serializesAsVoid, isBeingRendered, } from './whatwg-html.js';
import { htmlNamespace, } from './whatwg-infra.js';
...
export function processAndConsumeFragmentDirective(url) {
    ...
    let documentFragmentDirective = null;
...
export function parseTextDirective(textDirectiveInput) {
...
export function isValidFragmentDirective(input) {
    // TODO (use PEG.js?)
    return true; // TEMP
}
export function isTextFragmentDirective(input) {
    // TODO (use PEG.js?)
...
export function scrollToTheFragment(indicatedPart) {
...
export function indicatedPartOfTheDocument_beginning({ document, documentFragmentDirective, documentAllowTextFragmentDirective }) {
    let documentIndicatedPart = undefined;
...
export function firstCommonAncestor(nodeA, nodeB) {
    ...
    while (!isShadowIncludingInclusiveAncestor(commonAncestor, /* of */ nodeB))
        commonAncestor = shadowIncludingParent(commonAncestor);
...
export function shadowIncludingParent(node) {
...
export function scrollDomRectIntoView(boundingBox, behavior, block, inline, startingElement) {
    // “OMITTED”
    ...
    function applyScrollLogicalPosition({ position, boundingBoxRelativeEdgeBegin, boundingBoxRelativeEdgeEnd, boundingBoxSize, scrollBoxAbsoluteEdgeBegin, scrollBoxSize, }) {
...
export function scrollElementIntoView(element, behavior, block, inline) {
...
export function scrollRangeIntoView(range, behavior, block, inline, containingElement) {
...
export function processFragmentDirective(fragmentDirectiveInput, document) {
...
export function findRangeFromTextDirective(parsedValues, document) {
...
export function advanceRangeStartToNextNonWhitespacePosition(range) {
    ...
    while (!range.collapsed) {
        ...
        if (partOfNonSearchableSubtree(node)) {
            ...
            range.setStart(nextNodeInShadowIncludingTreeOrderThatIsNotAShadowIncludingDescendantOf(node), // XXX Can we be sure there is a next node? Asserting it here.
            0);
            // 2. “Continue.”
            continue;
        }
        ...
        if (!isVisibleTextNode(node)) {
            ...
            range.setStart(nextNodeInShadowIncludingTreeOrder(node), // XXX Can we be sure there is a next node? Asserting it here.
            0);
            // 2. “Continue.”
            continue;
        }
        ...
        if (substringData(node, offset, 6) === '&nbsp;') { // XXX Is node guaranteed to be CharacterData? Not clear in spec.
        ...
        else if (substringData(node, offset, 5) === '&nbsp') { // XXX Is node guaranteed to be CharacterData? Not clear in spec.
...
export function findStringInRange(query, searchRange, wordStartBounded, wordEndBounded) {
    ...
    while (!searchRange.collapsed) {
        ...
        if (partOfNonSearchableSubtree(curNode)) {
            ...
            searchRange.setStart(nextNodeInShadowIncludingTreeOrderThatIsNotAShadowIncludingDescendantOf(curNode), // XXX Can we be sure there is a next node? Asserting it here.
            0);
            // 2. “Continue.”
            continue;
        }
        ...
        if (!isVisibleTextNode(curNode)) {
            ...
            while (curNode && curNode.nodeType === Node.DOCUMENT_TYPE_NODE)
                curNode = nextNodeInShadowIncludingTreeOrder(curNode);
            searchRange.setStart(curNode, // XXX Can we be sure there is a next node? Asserting it here.
            0);
            // 2. “Continue.”
            continue;
        }
        ...
        while (curNode && isShadowIncludingDescendant(curNode, /* of */ blockAncestor)
...
export function isSearchInvisible(node) {
...
export function partOfNonSearchableSubtree(node) {
    let curNode = node;
    while (curNode) {
        if (isSearchInvisible(curNode))
            return true;
...
export function hasBlockLevelDisplay(node) {
...
export function nearestBlockAncestorOf(node) {
...
export function findARangeFromANodeList(queryString, searchRange, nodes, wordStartBounded, wordEndBounded) {
    ...
    while (matchIndex === null) {
        ...
        start = getBoundaryPointAtIndex(matchIndex, nodes, false);
        ...
        if (wordStartBounded && !isAtWordBoundary(matchIndex, searchBuffer, languageOf(start[0]))
...
export function isAtWordBoundary(position, text, locale) {
...
export function browserSupportsTextFragments() {
